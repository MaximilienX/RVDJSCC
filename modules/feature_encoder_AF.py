...
class Equalizer(nn.Module):
    ...
    def _regular_arch(self, c_in, c_feat, c_out):
        # Define the layers of the Equalizer, similar to FeatureDecoderAF without upsampling
        self.layers = nn.ModuleDict({
            'a1': AttentionBlock(c_in),  # Attention Block with SNR

            'rb1': ResidualBlock(in_ch=c_in, out_ch=c_feat),
            'af1': AFModule(c_in=c_feat),

            'rb2': ResidualBlock(in_ch=c_feat, out_ch=c_feat),
            'af2': AFModule(c_in=c_feat),
            
            'a2': AttentionBlock(c_feat),  # Attention Block with SNR

            'rb3': ResidualBlock(in_ch=c_feat, out_ch=c_feat),
            'af3': AFModule(c_in=c_feat),

            'rb4': ResidualBlock(in_ch=c_feat, out_ch=c_out),
            'af4': AFModule(c_in=c_out)
        })
    ...
    def forward(self, x, snr):
        ...
        # Process through layers, including SNR passing through attention blocks
        for key in self.layers:
            if key[:2] == 'af':  # Apply Attention with SNR
                x = self.layers[key](x, snr)
            else:
                x = self.layers[key](x)  # Apply residual blocks and others
